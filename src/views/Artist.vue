...
<!--      artist {{this.$route.params.id}}-->
      ...
      <el-tabs v-model="active">
        ...
        <el-tab-pane label="艺人信息" name="third">
          <ArtistDesc :introduction="artist.introduction" :experience="artist.experience"></ArtistDesc>
        </el-tab-pane>
      </el-tabs>
...
export default {
  ...
  methods: {
    getHot () {
      this.$axios.get(this.$localMusicApi + '/artist/hot?id=' + this.$route.params.id).then(this.getHotSucc)
    },
    getHotSucc (res) {
      this.hotSongs = res.data.data
    },
    getPlaylistByArtistId () {
      this.$axios.get(this.$localMusicApi + '/artist?id=' + this.$route.params.id).then(this.getPlaylistByArtistIdSucc)
    },
